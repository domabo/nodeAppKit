...
exports.invokeContext = function invokeContext(context, callBack) {
    ...
    console.log("loading"  + 'request-'+context["server.appId"]);
    
    
    // TO DO: LOAD REQUEST COOKIES FROM CACHE
    //SCHEDULE ACTUAL WORK ASYNC
   process.nextTick(function() {
                     console.log("loaded"  + 'request-'+context["server.appId"]);
                     
                     private_OwinEventHost.emit('request-'+context["server.appId"], context, function(err, value)
                                              {
...
function private_addDefaultFields(context) {
    context.tokenSource = new cancellationTokenSource();
    ...
    context["nodeAppKit.callCancelledSource"] = context.tokenSource;
    context["owin.Version"] = "1.0";
    context["owin.callCancelled"] = context.tokenSource.token;
